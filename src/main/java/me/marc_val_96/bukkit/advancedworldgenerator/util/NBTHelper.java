...
public class NBTHelper {
        ...
        for (int i = 0; i < nmsListTag.size(); i++) {
            switch (listType) {
                ...
                case TAG_Compound:
                    ...
                    break;
                default:
                    AWG.log(LogMarker.INFO, "Cannot convert list subtype {} from it's NMS value", listType);
                    break;
            }
        }
        return listTag;
    ...
    private static Object getValueFromNms(NBTBase nmsTag) {
        ...
        switch (type) {
            case TAG_Byte:
                return ((NBTTagByte) nmsTag).g();
            case TAG_Short:
                return ((NBTTagShort) nmsTag).f();
            case TAG_Int:
                return ((NBTTagInt) nmsTag).e();
            case TAG_Long:
                return ((NBTTagLong) nmsTag).d();
            case TAG_Float:
                return ((NBTTagFloat) nmsTag).i();
            case TAG_Double:
            ...
            case TAG_Int_Array:
                return ((NBTTagIntArray) nmsTag).d();
            default:
