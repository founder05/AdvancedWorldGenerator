...
import com.marc_val_96.advancedworldgenerator.*;
import com.marc_val_96.advancedworldgenerator.configuration.*;
...
import me.marc_val_96.bukkit.advancedworldgenerator.generator.AWGInternalChunkGenerator;
import me.marc_val_96.bukkit.advancedworldgenerator.generator.AWGWorldChunkManager;
import me.marc_val_96.bukkit.advancedworldgenerator.generator.BukkitVanillaBiomeGenerator;
import me.marc_val_96.bukkit.advancedworldgenerator.generator.structures.*;
import me.marc_val_96.bukkit.advancedworldgenerator.util.NBTHelper;
...
public class BukkitWorld implements LocalWorld {
    private static final int MAX_BIOMES_COUNT = AWG.SUPPORTED_BIOME_IDS;
    ...
    public AWGStrongholdGen strongholdGen;
    public AWGVillageGen villageGen;
    public AWGMineshaftGen mineshaftGen;
    public AWGRareBuildingGen rareBuildingGen;
    public AWGMansionGen mansionGen;
    public AWGNetherFortressGen netherFortressGen;
    public AWGOceanMonumentGen oceanMonumentGen;
    // Initially false, set to true when enabled once
    private boolean initialized;
    private AWGChunkGenerator generator;
    ...
    private void replaceBlocks(Chunk rawChunk) {
        ...
        for (ChunkSection section : sectionsArray) {
            ...
            for (int sectionX = 0; sectionX < endXInChunk; sectionX++) {
                for (int sectionZ = 0; sectionZ < endZInChunk; sectionZ++) {
    ...
    private Chunk getChunk(int x, int y, int z) {
        if (y < AWG.WORLD_DEPTH || y >= AWG.WORLD_HEIGHT)
            return null;
    ...
    public LocalMaterialData getMaterial(int x, int y, int z) {
        Chunk chunk = this.getChunk(x, y, z);
        if (chunk == null || y < AWG.WORLD_DEPTH || y >= AWG.WORLD_HEIGHT) {
    ...
    public AWGChunkGenerator getChunkGenerator() {
        return this.generator;
    }

    public void setChunkGenerator(AWGChunkGenerator _generator) {
    ...
    public void enable(org.bukkit.World world) {
        ...
        biomeGenerator = AWG.getBiomeModeManager().createCached(biomeModeClass, this);
        ...
        if (!initialized) {
            ...
            switch (this.settings.getWorldConfig().ModeTerrain) {
                case Normal:
                case OldGenerator:
                    this.strongholdGen = new AWGStrongholdGen(settings);
                    this.villageGen = new AWGVillageGen(settings);
                    this.mineshaftGen = new AWGMineshaftGen();
                    this.rareBuildingGen = new AWGRareBuildingGen(settings);
                    this.mansionGen = new AWGMansionGen(settings);
                    this.netherFortressGen = new AWGNetherFortressGen();
                    this.oceanMonumentGen = new AWGOceanMonumentGen(settings);

                    // Inject our own ChunkGenerator
                    injectInternalChunkGenerator(new AWGInternalChunkGenerator(this, generator));
                case NotGenerate:
                case TerrainTest:
    ...
    private void injectWorldChunkManager(BiomeGenerator biomeGenerator) {
        if (biomeGenerator instanceof BukkitVanillaBiomeGenerator) {
            ...
            ReflectionHelper.setValueInFieldOfType(this.world.worldProvider,
                    WorldChunkManager.class, new AWGWorldChunkManager(this, biomeGenerator));
        }
    }
    ...
    private Entity createEntity(MinecraftKey entityResource, NBTTagCompound optionalMeta) {
        ...
        if (entityClass == null) {
            AWG.log(LogMarker.WARN, "Unknown entity name: {}", entityResource);
            return null;
        }
