...
public final class ServerConfigProvider implements ConfigProvider {
    ...
    private void loadCustomObjects() {
        ...
        if (!FileHelper.migrateFolder(oldWorldObjectsDir, worldObjectsDir)) {
            AWG.log(LogMarker.WARN, "Failed to move old world"
        ...
        Map<String, CustomObjectLoader> objectLoaders = AWG.getCustomObjectManager().getObjectLoaders();

        customObjects = new CustomObjectCollection(objectLoaders, worldObjectsDir);
        customObjects.setFallback(AWG.getCustomObjectManager().getGlobalObjects());
        AWG.log(LogMarker.INFO, "{} world custom objects loaded.", customObjects.getAll().size());
    }
    ...
    private void loadBiomes(SettingsMap worldConfigSettings) {
        ...
        // AWG/worlds/<WorldName>/<WorldBiomes/
        biomeDirs.add(new File(settingsDir, correctOldBiomeConfigFolder(settingsDir)));
        // AWG/GlobalBiomes/
        biomeDirs.add(new File(AWG.getEngine().getTCDataFolder(), PluginStandardValues.BiomeConfigDirectoryName));

        FileHelper.makeFolders(biomeDirs);

        // Build a set of all biomes to load
        Collection<BiomeLoadInstruction> biomesToLoad = new HashSet<BiomeLoadInstruction>(world.getDefaultBiomes());
        ...
        // Load all files
        BiomeConfigFinder biomeConfigFinder = new BiomeConfigFinder(worldConfig, AWG.getPluginConfig().biomeConfigExtension);
        ...
        AWG.log(LogMarker.INFO, "{} biomes Loaded", biomesCount);
        AWG.log(LogMarker.DEBUG, "{}", loadedBiomeNames);
    }
    ...
    private String indexSettings(Map<String, BiomeConfig> loadedBiomes) {
        ...
        loadedBiomeList.sort(new Comparator<BiomeConfig>() {
            @Override
        ...
        for (BiomeConfig biomeConfig : loadedBiomeList) {
            ...
            if (!biomeConfig.replaceToBiomeName.isEmpty()) {
                ...
                if (replaceToConfig == null) {
                    AWG.log(LogMarker.WARN, "Invalid ReplaceToBiomeName in biome {}: biome {} doesn't exist", biomeConfig.getName(),
                            biomeConfig.replaceToBiomeName);
                    biomeConfig.replaceToBiomeName = "";
                } else if (!replaceToConfig.replaceToBiomeName.isEmpty()) {
                    AWG.log(LogMarker.WARN, "Invalid ReplaceToBiomeName in biome {}: biome {} also has a ReplaceToBiomeName value",
            ...
            // Indexing BiomeColor
            if (this.worldConfig.biomeMode == AWG.getBiomeModeManager().FROM_IMAGE) {
    ...
    private void processInheritance(Map<String, BiomeConfigStub> biomeConfigStubs, BiomeConfigStub biomeConfigStub, int currentDepth) {
        ...
        if (extendedBiomeConfig == null) {
            AWG.log(LogMarker.WARN, "The biome {} tried to extend the biome {}, but that biome doesn't exist.",
    ...
    private String correctOldBiomeConfigFolder(File settingsDir) {
        ...
        if (oldBiomeConfigs.exists()) {
            if (!oldBiomeConfigs.renameTo(new File(settingsDir, biomeFolderName))) {
                AWG.log(LogMarker.WARN, "========================");
                AWG.log(LogMarker.WARN, "Fould old `BiomeConfigs` folder, but it could not be renamed to `", biomeFolderName, "`!");
                AWG.log(LogMarker.WARN, "Please rename the folder manually.");
                AWG.log(LogMarker.WARN, "========================");
