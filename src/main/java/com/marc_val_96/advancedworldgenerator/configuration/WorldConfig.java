...
public class WorldConfig extends ConfigFile {
    ...
    public WorldConfig(File settingsDir, SettingsMap settingsReader, LocalWorld world,
        ...
        for (Iterator<Entry<String, Integer>> it = customBiomeGenerationIds.entrySet().iterator(); it.hasNext(); ) {
            ...
            if (DefaultBiome.Contain(biomeName)) {
                AWG.log(LogMarker.WARN, "CustomBiomes only accepts custom biomes,"
    ...
    protected void correctSettings() {
        ...
        if (biomeMode == AWG.getBiomeModeManager().FROM_IMAGE) {
            File mapFile = new File(settingsDir, imageFile);
            if (!mapFile.exists()) {
                AWG.log(LogMarker.WARN, "Biome map file not found. Switching BiomeMode to Normal");
                biomeMode = AWG.getBiomeModeManager().NORMAL;
            }
        }

        imageFillBiome = (DefaultBiome.Contain(imageFillBiome) || customBiomeGenerationIds.containsKey(imageFillBiome)) ? imageFillBiome : WorldStandardValues.IMAGE_FILL_BIOME.getDefaultValue();
        ...
        if (biomeMode == AWG.getBiomeModeManager().OLD_GENERATOR && ModeTerrain != TerrainMode.OldGenerator) {
            AWG.log(LogMarker.WARN, "Old biome generator works only with old terrain generator!");
            biomeMode = AWG.getBiomeModeManager().NORMAL;
    ...
    protected void readConfigSettings(SettingsMap reader) {
        ...
        this.biomeMode = AWG.getBiomeModeManager().getBiomeManager(reader.getSetting(WorldStandardValues.BIOME_MODE));
    ...
    protected void writeConfigSettings(SettingsMap writer) {
        ...
        writer.putSetting(WorldStandardValues.BIOME_MODE, AWG.getBiomeModeManager().getName(biomeMode),
