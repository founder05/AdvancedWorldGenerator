...
public final class BiomeGroupManager
{
    ...
    public void registerGroup(BiomeGroup newGroup)
    {
        if (isRoomForMoreGroups())
        {
            ...
            {
                AWG.log(LogMarker.WARN, "Two biome groups have the same name \"{}\". Removing the second one.",
                        newGroup.getName());
                AWG.printStackTrace(LogMarker.WARN, new Exception());
            } else {
            ...
            AWG.log(LogMarker.WARN, "Biome group \"{}\" could not be added. Max biome group count reached.", newGroup.getName());
        }
    }
    ...
    {
        Integer[] totalRarity = map.keySet().toArray(new Integer[0]);
    ...
    public void filterBiomes(Set<String> keySet) {
    }
}
