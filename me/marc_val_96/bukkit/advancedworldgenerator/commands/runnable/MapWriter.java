...
public class MapWriter implements Runnable {
    ...
    private int[] getColors(World world) {
        AWG.log(LogMarker.TRACE, "BukkitWorld::UUID:: {}", world.getDataManager().getUUID());
        LocalWorld bukkitWorld = WorldHelper.toLocalWorld(world);
        if (bukkitWorld == null) {
            AWG.log(LogMarker.WARN, "BukkitWorld is null :: Make sure you add `{}` to bukkit.yml", world.getWorld()
                    .getName());
        ...
        AWG.log(LogMarker.TRACE, "BukkitWorld settings biomes.length::{}", biomes.length);
    ...
    public void run() {
                ...
                for (int x1 = 0; x1 < 16; x1++) {
                    for (int z1 = 0; z1 < 16; z1++) {
                        ...
                        try {
                            ...
                            AWG.log(LogMarker.TRACE, "BiomeBuff Idx::{}<{}x/{}z>, Len::{}, ID::{} | Colors Len::{}",
                                    arrayPosition, x1, z1, biomeBuffer.length, WorldHelper.getGenerationId(biomeBuffer[arrayPosition]), colors.length);
                        }
                    }
                }
            }
        ...
        try {
            ...
            AWG.log(LogMarker.ERROR, "Failed to write image.");
            AWG.printStackTrace(LogMarker.ERROR, e);
        }
