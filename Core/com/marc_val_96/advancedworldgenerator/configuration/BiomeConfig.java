...
import com.marc_val_96.advancedworldgenerator.customobjects.UseBiome;
...
public class BiomeConfig extends ConfigFile {
    ...
    protected void readConfigSettings(SettingsMap settings) {
        ...
        this.surfaceBlock = settings.getSetting(BiomeStandardValues.SURFACE_BLOCK,
                AWG.toLocalMaterialData(defaultSettings.defaultSurfaceBlock, 0));
        this.groundBlock = settings.getSetting(BiomeStandardValues.GROUND_BLOCK,
                AWG.toLocalMaterialData(defaultSettings.defaultGroundBlock, 0));
    ...
    protected void correctSettings() {
        ...
        this.replaceToBiomeName = (DefaultBiome.Contain(this.replaceToBiomeName) || this.worldConfig.customBiomeGenerationIds.containsKey(
                this.replaceToBiomeName)) ? this.replaceToBiomeName : "";

        this.riverBiome = (DefaultBiome.Contain(this.riverBiome) || this.worldConfig.customBiomeGenerationIds.containsKey(this.riverBiome)) ? this.riverBiome
                : "";
    }
    ...
    protected void renameOldSettings(SettingsMap settings) {
        ...
        if (replacedBlocksValue.contains("=")) {
            ...
            List<ReplacedBlocksMatrix.ReplacedBlocksInstruction> output = new ArrayList<ReplacedBlocksMatrix.ReplacedBlocksInstruction>();

            for (String replacedBlock : values) {
                try {
                    LocalMaterialData fromId = AWG.readMaterial(replacedBlock.split("=")[0]);
                    ...
                    if (start != -1 && end != -1) {   // Found height settings
                        ...
                        to = AWG.readMaterial(rest.substring(0, start));
